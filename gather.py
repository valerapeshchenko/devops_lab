...
def general_function():
	...
	p = Popen('pyenv virtualenvs', shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
	output = p.stdout.read().decode('utf-8')
	strings = re.findall(r'(?<=\s\s).+', output)
	d["virtualenv"] = strings[0]

	p = Popen('which pip', shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
	output = p.stdout.read().decode('utf-8')
	...
	d["version"] = str(sys.version_info[0]) + "." + str(sys.version_info[1]) + "." + str(sys.version_info[2])
	
	p = Popen('pip list', shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
	output = p.stdout.read().decode('utf-8')
